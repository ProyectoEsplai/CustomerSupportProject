...
from app.documents import cargar_corpus
from app.graph import (
    PasoRecorrido,
    RespuestaSoporte,
    escalar_a_humano,
    procesar_consulta,
    procesar_consulta_n2,
)
from app.ollama_client import OllamaError, estado_ollama
...
PREGUNTAS_FRECUENTES = [
    {"numero": "01", "label": "Instalación sin técnico", "pregunta": "¿Puedo instalar yo mismo el router Smart WiFi 6 o necesito técnico?"},
    {"numero": "02", "label": "Cambio desde ONT separada", "pregunta": "Si en mi instalación antigua hay una ONT separada, ¿qué hago al cambiar al Smart WiFi 6?"},
    {"numero": "03", "label": "Primera conexión WiFi", "pregunta": "¿Cómo conecto un móvil, tablet o portátil por WiFi la primera vez?"},
    {"numero": "04", "label": "Conexión por código QR", "pregunta": "¿Puedo conectarme escaneando el código QR del router?"},
    {"numero": "05", "label": "Conexión por cable Ethernet", "pregunta": "¿Puedo conectar un equipo por cable Ethernet?"},
    {"numero": "06", "label": "Nombre WiFi y claves", "pregunta": "¿Dónde está el nombre del WiFi, la clave y la contraseña de acceso al router?"},
    {"numero": "07", "label": "Luces LED del router", "pregunta": "¿Qué significan las luces LED del router?"},
    {"numero": "08", "label": "Internet rojo parpadeando", "pregunta": "¿Qué hago si la luz de Internet está roja parpadeando?"},
    {"numero": "09", "label": "Internet rojo fijo", "pregunta": "¿Qué hago si la luz de Internet está roja fija?"},
    {"numero": "10", "label": "Reinicio y configuración", "pregunta": "¿Reiniciar el router borra mi configuración?"},
    {"numero": "11", "label": "Reiniciar correctamente", "pregunta": "¿Cómo reinicio correctamente el router?"},
    {"numero": "12", "label": "Entrar en configuración local", "pregunta": "¿Cómo entro en la configuración local del router?"},
    {"numero": "13", "label": "Compatibilidad app Smart WiFi", "pregunta": "¿Mi router Smart WiFi 6 es compatible con la app Smart WiFi?"},
    {"numero": "14", "label": "Acceso a app Smart WiFi", "pregunta": "¿Cómo accedo a la app Smart WiFi?"},
    {"numero": "15", "label": "Dispositivos conectados", "pregunta": "¿Cómo veo los dispositivos conectados?"},
    {"numero": "16", "label": "Velocidad y cobertura", "pregunta": "¿Cómo mido velocidad y cobertura desde la app Smart WiFi?"},
    {"numero": "17", "label": "Cambiar nombre WiFi", "pregunta": "¿Cómo cambio el nombre del WiFi?"},
    {"numero": "18", "label": "Cambiar contraseña WiFi", "pregunta": "¿Cómo cambio la contraseña del WiFi?"},
    {"numero": "19", "label": "Red WiFi de invitados", "pregunta": "¿Cómo creo una red WiFi de invitados?"},
    {"numero": "20", "label": "Pausar o bloquear dispositivo", "pregunta": "¿Cómo pauso o bloqueo un dispositivo?"},
]
...
def comprobar_ollama() -> dict[str, object]:
    return estado_ollama()
...
def procesar_y_guardar(pregunta: str) -> None:
    ...
    try:
        ...
        texto = (
            "No he podido completar la respuesta porque Ollama no está disponible.\n\n"
            f"Detalle: {exc}"
        )
        st.session_state.mensajes.append(
            {"role": "assistant", "content": texto, "nivel": "Sistema", "respuesta": None}
        )
    except Exception as exc:  # pragma: no cover - visible en la UI de clase
        texto = f"Se produjo un error preparando el soporte: {exc}"
        st.session_state.mensajes.append(
            {"role": "assistant", "content": texto, "nivel": "Sistema", "respuesta": None}
        )
...
def revisar_no_solucionada() -> None:
    respuesta_anterior: RespuestaSoporte | None = st.session_state.ultima_respuesta
    ...
    try:
        if respuesta_anterior.nivel == "N1":
            nueva_respuesta = procesar_consulta_n2(pregunta)
        elif respuesta_anterior.nivel == "N2":
            nueva_respuesta = escalar_a_humano(
                pregunta,
                "El usuario indicó que N2 no solucionó la consulta",
            )
        else:
            nueva_respuesta = escalar_a_humano(
                pregunta,
                "La consulta ya estaba derivada a atención humana",
            )
        ...
        st.session_state.mensajes.append(
            {
                "role": "assistant",
                "content": (
                    "No he podido revisar la consulta porque Ollama no está disponible.\n\n"
                    f"Detalle: {exc}"
                ),
                "nivel": "Sistema",
                "respuesta": None,
...
def render_recorrido(respuesta: RespuestaSoporte | None) -> None:
    if not respuesta:
...
def render_detalle(respuesta: RespuestaSoporte | None) -> None:
    if not respuesta:
        return
    ...
    with st.sidebar:
        ...
        st.markdown(
            ...
            <div class="contact-box">
                <strong>Asistente humano</strong><br>
                {html.escape(config.HUMAN_PHONE)}<br>
                {html.escape(config.HUMAN_EMAIL)}
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def render_resolution_actions() -> None:
    ...
    if respuesta is None or respuesta.nivel in {"Entrada", "Sistema"}:
        return
...
def _limpiar_respuesta_cliente(texto: str) -> str:
    marcador = "Fuentes consultadas:"
    if marcador in texto:
        return texto.split(marcador, 1)[0].strip()
    return texto
...
def main() -> None:
    ...
    if pregunta:
        with st.spinner("Procesando N1, N2 y escalado..."):
